...
Results table. Each row is the captured outcome for a single enrolment. result_id is the primary key, and enrolment_id is a required foreign key back to Event_Enrolments. Because the ERD models this relationship as one-to-one rather than one-to-many, enrolment_id also carries a uniqueness constraint on top of being a foreign key — that's the mechanism that actually enforces "at most one result per entry" at the database level, rather than relying on the application to check for duplicates. finish_time and gun_time are stored as time-interval values rather than plain text, so results can be sorted and compared numerically for leaderboard ranking. status follows the fixed results-status list described below.

General conventions used throughout the script:

Every primary key auto-increments, so the application never has to generate IDs itself.
Every foreign key is indexed, since these are the columns the API filters and joins on most often (e.g. "all categories for this event", "all enrolments for this user").
Deletion rules are chosen deliberately rather than left at the database default: child records that are meaningless without their parent (Categories, Routes, Event_Enrolments, Results) cascade on delete, while an Organiser's account is protected from deletion if they still own events, to avoid silently destroying event history.
All three status columns use check constraints tied to the fixed lists in Status Values, rather than allowing arbitrary text, to keep the data consistent for reporting and filtering.
